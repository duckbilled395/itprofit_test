...
const Form: FC = () => {
    ...
    const nameOnChangeHandler = React.useCallback((e: FormEvent<HTMLFormElement> | FormEvent<HTMLInputElement>) => {
        ...
        if (!re.test(String(target.value).toLowerCase()) || target.value === ' ') {
            ...
            if (target.value.split(' ')[0].length < 3 || target.value.split(' ')[0].length > 30) {
                ...
                typeof target.value.split(' ')[2] === 'undefined'
            ) {
                setNameError('')
            }
    ...
    const handleOnSubmit = (e: FormEvent<HTMLFormElement>, name: string, email: string,
                            phone: string, date: Date, message: string, nameError: string,
                            emailError: string, phoneError: string, messageError: string) => {
        e.preventDefault();
    ...
    return (
        <div className='wrapper'>
            ...
            <form className='form' id='form'
                  onSubmit={(e) => handleOnSubmit(e, name, email, phone, date, message, nameError, emailError, phoneError, messageError)}
                  noValidate>
